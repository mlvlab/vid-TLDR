...
def read_frames_decord(
    ...
    root_path, data_name = "/".join(video_path.split("/")[:-2]), f'{".".join(video_path.split("/")[-1].split(".")[:-1])}'
    raw_file_path = os.path.join(root_path, "raw", str(num_frames), data_name + ".pt")
    preprocess = os.path.exists(raw_file_path)
    if preprocess:
        frames = torch.load(raw_file_path)
    else:
