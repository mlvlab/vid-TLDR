...
from utils.vid_TLDR import vidTLDR
...
class Attention(nn.Module):
    ...
    def forward(self, x, size = None):
        ...
        if size is not None:
            attn = attn + size.log()[:, None, None, :, 0].detach()
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 attn_head_dim=None, i = 0, info = None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.gamma_1 is None:
            x_, attn = self.attn(self.norm1(x), size)
            x = x + self.drop_path(x_)
            x = vidTLDR(x, attn, self.info, self.i)
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
                 ...
                 use_checkpoint=False, checkpoint_num=0, ckpt_num_frame=-1, **kwargs):
        super().__init__()
        ...
        self.info = {
            "use"  : kwargs["vidTLDR_use"],
            "r"    : kwargs["vidTLDR_r"],
            "source_trace" : False, "source" : None, "size" : None
        }
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, i = i, info = self.info)
...
def build_vit(config, add_pool_norm=True):
    model = PretrainVisionTransformer(
        ...
        vidTLDR_use  = config.vision_encoder.vidTLDR_use,
        vidTLDR_r    = config.vision_encoder.vidTLDR_r,
    )
