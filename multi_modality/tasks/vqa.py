...
from models.umt_qa import UMT_QA
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'UMT_QA'))
    (
        model,
        model_without_ddp,
        optimizer,
    ...
    for epoch in range(start_epoch, config.scheduler.epochs):
        ...
        if config.get('no_test', False) and not config.evaluate:
            ...
            with torch.cuda.amp.autocast(enabled=config.fp16):
                model.vision_encoder.encoder.use_checkpoint = False
                gflops = flops(model, (1, config.num_frames_test, 3, 224, 224), "flops", fp16=True)
                model.vision_encoder.encoder.use_checkpoint = True
